...
window.user.position.set(0,0,0);
window.scene.add(window.user)
...
function AlignUser() {
    ...
    // pos.addScaledVector(cameraDirection, .725);
    // pos.addScaledVector(cameraUp, -1);
    window.user.position.copy(pos);
    window.user.position.needsUpdate = true;
}
    ...
    for (let i = 0; i < segments; i++) {
        for (let j = 0; j < segments; j++) {
            ...
            if (a >= 0 && b >= 0 && c >= 0 && d >= 0 &&
                ...
                map[partitionKey1].push(TriangleMesh(vertices, a, b, d));
                ...
                map[partitionKey2].push(TriangleMesh(vertices, b, c, d));
            } else {
...
USER: ${user.position.x}, ${user.position.y}, ${user.position.z}
TOUCHING: (${Array.from(camera.touches).length}) <table style="overflow: auto;wmax-width:100vw;">
	...
	${Array.from(camera.touches)
		...
		<td style="border: 1px solid rgba(255,255,11,0.5)">${o.position.x.toFixed(3)}, ${o.position.y.toFixed(3)}, ${o.position.z.toFixed(3)}</td>
		<td style="border: 1px solid rgba(255,11,255,0.5)">${camera.position.distanceTo(o.position)}</td>
	</tr>`).join('')}
</table>
`
}
