...
class Terrain {
    ...
    generateTerrain(centerX = 0, centerY = 0, centerZ = 0) {
        ...
        for (let i = 0; i < terrainGeo.attributes.normal.count; i++) {
            const nx = terrainGeo.attributes.normal.getX(i);
            const ny = terrainGeo.attributes.normal.getY(i);
            const nz = terrainGeo.attributes.normal.getZ(i);
            
            // If normals face downward, flip them
            if (ny < 0) {
                console.log("flipped")
                terrainGeo.attributes.normal.setXYZ(i, -nx, -ny, -nz);
            }
        }
    ...
    addTerrainFeature(i, j, segmentSize, vertices, indices) {
        ...
        [t1, t2].forEach((triangleMesh) => {
            ...
            if (HOUSE || (!COVE && !CLIFF && !HOUSE && !DOCK && !BOARDWALK)) {
                ...
                const bladeCount = 0//window.innerWidth < 800 ? 100 : 300;
...
class UserController {
    ...
    handleCollision() {
        ...
        for (const direction of directions) {
            const downRaycaster = new THREE.Raycaster(this.camera.position.clone(), direction);
    
            // Logging the camera position and ray direction
            console.log("Camera Position:", this.camera.position);
            console.log("Ray Direction (Down):", direction);

            const intersections = downRaycaster.intersectObject(terrain.terrain, true);

            // Log if any intersections are found
            if (intersections.length > 0) {
                const closestIntersection = intersections[0];  // Closest intersection below the camera

                // Log details of the closest intersection
                console.log("Intersection Found:");
                console.log("Intersection Distance:", closestIntersection.distance);
                console.log("Intersection Point:", closestIntersection.point);
                console.log("Terrain Normal at Intersection:", closestIntersection.face.normal);

                // Check if camera should "land"
                if (closestIntersection.distance < 61.2) {  // Adjust this threshold based on height from which camera should "land"
                    this.camera.position.y = closestIntersection.point.y + 0.6;  // Offset to prevent sinking into the ground
                    this.camera.velocity.y = 0;
                    this.isJumping = false;
                    this.grounded = true;
                    console.log("Camera grounded. Adjusting position to:", this.camera.position.y);
                }
            } else {
                this.grounded = false;
                //console.log("No intersection detected below the camera.");
            }
            ...
            // const raycaster = new THREE.Raycaster(this.camera.position, direction.normalize());

            // const intersectsTrees = raycaster.intersectObjects(terrain.trees.flatMap(tree => [tree.trunk, tree.foliage]), true);
    ...
    applyGravity() {
        ...
        // const downIntersection = this.findClosestIntersection(raycaster);

        // if (downIntersection && downIntersection.distance < 1) {
        //     this.intersections.push(downIntersection)

        //     var newY = downIntersection.point.y + 1

        //     this.camera.position.y = newY;
        //     this.floor = downIntersection.point.y + 1
    ...
    findClosestIntersection(raycaster) {
        ...
        for (let mesh of [terrain.terrain]) {
